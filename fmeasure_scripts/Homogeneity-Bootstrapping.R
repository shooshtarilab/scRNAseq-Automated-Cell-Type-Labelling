...
colnames(df) <- c("dataset", "algorithm", "homogeneity_mean", "homogeneity_low_CI", "homogeneity_high_CI")
write.table(df, file="../Rdata/Homogeneity_bootstrapping.tsv", row.names = FALSE, sep='\t') 
df <- as.data.frame(df)
#df$metric = "Homogeneity"
#write.table(df, file="Homogeneity_bootstrapping_temp.tsv", row.names = FALSE,sep='\t')
#df <- read.csv("Homogeneity_bootstrapping_temp.tsv", stringsAsFactors = FALSE,sep='\t')
#colnames(df) <- c("dataset", "algorithm", "homogeneity_mean", "homogeneity_low_CI", "homogeneity_high_CI")
#df$metric = "homogeneity"
...
df$score = 1
for (dataset in all.datasets) {
  df[df$dataset == dataset,"score"] <- rank(df[df$dataset == dataset,"homogeneity_mean"])
}
