...
for (dataset in all.datasets) {
  ...
  for (alg in intersect(all.algorithms, colnames(data))) {
    ...
    bootstrap.func <- function(x) {
      ...
      C1 <- C[indx]
      K1 <- K[indx]
      return(F.Measure.Func(C1, K1)$F.measure)
    }
  ...
  write.csv(df, file="dataframe_results/FMeasure_bootstrap.csv", row.names = FALSE)
}

df <- as.data.frame(df)
colnames(df) <- c("dataset", "algorithm", "FMeasure", "FMeasure_low_CI", "FMeasure_high_CI")
write.csv(df, file="FMeasure_bootstrap.csv", row.names = FALSE)

df <- read.csv("FMeasure_bootstrap.csv", stringsAsFactors = FALSE)
...
write.csv(df, file="dataframe_results/FMeasure_bootstrapping.csv", row.names = FALSE)
