...
##################################################################
AMI <- low.CI <- high.CI <- matrix(0, nrow=length(all.algorithms), ncol=length(all.datasets), dimnames=list(all.algorithms, all.datasets))
...
for (dataset in all.datasets) {
  ...
  for (alg in intersect(all.algorithms, colnames(data))) {
    ...
    AMI[alg, dataset] <- round(CI[2], 2)
    
    x <- c(dataset, alg, AMI[alg, dataset], low.CI[alg, dataset],  high.CI[alg, dataset])
    df <- rbind(df, x)
  }
  write.csv(df, file="dataframe_results/AMI_bootstrapping.csv", row.names = FALSE)
}

df <- as.data.frame(df)
colnames(df) <- c("dataset", "algorithm", "AMI", "AMI_low_CI", "ARI_high_CI")
write.csv(df, file="AMI_bootstrapping.csv", row.names = FALSE)

df <- read.csv("AMI_bootstrapping.csv", stringsAsFactors = FALSE)
