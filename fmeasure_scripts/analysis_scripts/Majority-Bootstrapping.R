...
for (dataset in all.datasets) {
  ...
  write.csv(df, file="dataframe_results/majority_bootstrapping.csv", row.names = FALSE)
}

df <- as.data.frame(df)
colnames(df) <- c("dataset", "algorithm", "Majority", "Majority_low_CI", "Majority_high_CI")
write.csv(df, file="majority_bootstrapping.csv", row.names = FALSE)


df <- read.csv("majority_bootstrapping.csv", stringsAsFactors = FALSE)
...
write.csv(df, file="dataframe_results/majority_bootstrapping.csv", row.names = FALSE)
df <- read.csv("dataframe_results/majority_bootstrapping.csv", stringsAsFactors = FALSE)
colnames(df) <- c("dataset", "algorithm", "metric_mean", "metric_low_CI", "metric_high_CI")
df$metric = "majority"
