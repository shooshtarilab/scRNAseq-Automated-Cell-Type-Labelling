...
for (dataset in all.datasets) {
  ...
  colnames(df) <- c("dataset", "algorithm", "AMI_mean", "AMI_low_CI", "ARI_high_CI")
  write.table(df, file="../Rdata/AMI_bootstrapping.tsv", row.names = FALSE, sep='\t')
}
...
df$score = 1
for (dataset in all.datasets) {
  df[df$dataset == dataset,"score"] <- rank(df[df$dataset == dataset,"AMI_mean"])
}
